...
class ConceptNet:
	...
	# BFS to link 2 concepts
	def bfs_outbonds(self, concept1, concept2):
		# Vistied and path queue
		visited = []
		queue = [[concept1]]
		no_outbonds = []
		...
		if concept1 == concept2:
			print(f'{concept1} and {concept2} are the same concept, aborting BFS...\n')
			return []
		...
		while queue:
			...
			# Checks if visited already
			if curr_concept not in visited:

				# Gets outbond neighbors if concept has outbonds
				if curr_concept in self.outbonds.keys():
					outbond_neighbors = list(self.outbonds[curr_concept].keys())

					# Goes through outbond neighbors
					for neighbor in outbond_neighbors:
						new_path = list(path)
						new_path.append(neighbor)
						queue.append(new_path)

						# Checks if we reached concept2, our goal
						if neighbor == concept2:
							# return new_path
							# print(f'found link: {concept1}, {concept2}')
							all_paths.append(new_path)

				# No outbonds for current concept
				else:
					no_outbonds.append(curr_concept)
					# print(f'{curr_concept} has no outbonds.')

				# Adds curr_concept to visited
				visited.append(curr_concept)

		# Could not connect two concepts through outbonds
		if not all_paths:
			print(f'Was unable to connect {concept1} to {concept2} through outbonds...')
		return all_paths